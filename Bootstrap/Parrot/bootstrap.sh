...
wget -q http://archive.parrotsec.org/parrot/misc/archive.gpg -O "$2/etc/apt/trusted.gpg.d/parrot-archive-key.asc"

#tar the rootfs
cd "$2"
if [ -f "../parrot-rootfs-$1.tar.xz" ]; then
    rm -f "../parrot-rootfs-$1.tar.xz"
fi
if [ -d "dev" ]; then
    rm -rf dev/*
fi
